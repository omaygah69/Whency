...
export default function AddPage() {
		...
		try {
			...
			const isDuplicate = passwordEntries.some(
				(entry: { name: string; hashedPassword: string }) =>
					entry.name === name &&
					bcrypt.compareSync(password, entry.hashedPassword)
			);
	...
	return (
		<View className="flex-1 justify-center items-center p-20 bg-bgColor">
			...
			>
				<Text className="text-bgColor text-lg font-bold text-center">
					Save Password
				</Text>
			</TouchableOpacity>
			...
			>
				<Text style={{ fontSize: 24, color: "lightgreen", fontWeight: "bold" }}>
					✔️ Saved!
				</Text>
			</Animated.View>
